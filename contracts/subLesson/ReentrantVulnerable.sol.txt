...
contract ReentrantVulnerable {
    ...
    function withdraw() public {
        ...
        // Vulnerbility: Updating storage after transfer
...
contract Attack {
    ...
    function attack() external payable {
        ...
        /*When calling withdraw, the target contract calls this contract to transfer eth, so if there's a `fallback` implemented, it 
            will be executed DURING the execution of `withdraw`
            And if fallback calls `withdraw`, the target contract will transfer ETH to here BEFORE the balance is set to 0(target sees the caller still has balance and will transfer the amount)
        */
        reentrantVulnerable.withdraw();
    }
